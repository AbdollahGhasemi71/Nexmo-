...
@section('breadcrumb')
    <li><a href="{{ route('courses.index') }}" title="دوره ها"> ویرایش دوره ها</a></li>
    ...
    <div class="row no-gutters  ">
        <div class="col-12 bg-white">
            ...
            <form action="{{ route('courses.update',$course->id) }}" class="padding-30" method="post"
                ...
                <x-input type="text" name="slug" value="{{$course->slug}}" placeholder="نام انگلیسی دوره"
                         class="text-left" required/>


                <div class="d-flex multi-text">
                    <x-input type="text" class="text-left mlg-15" value="{{$course->priority}}" name="priority"
                             placeholder="ردیف دوره"/>
                    <x-input type="text" placeholder="مبلغ دوره" value="{{$course->price}}" name="price"
                             class="text-left" required/>
                    <x-input type="number" placeholder="درصد مدرس" value="{{$course->percent}}" name="percent"
                             class="text-left" required/>
                </div>
                ...
                <x-textarea placeholder="توضیحات دوره" name="body" value="{{$course->body}}" selected/>
                <br>
